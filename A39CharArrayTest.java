...
System.out.println(nums);
		...
		System.out.println("2.char열을 문자열배열로 변환");
		tempArr[5]='*';
		temp= String.valueOf(tempArr);
		System.out.println(temp + ", 문자열길이 : "+temp.length());
		System.out.print(tempArr);		//tempArr 을 출력을 위해 자동으로 String변환
		System.out.println(", 배열길이 : "+tempArr.length);
		System.out.println(tempArr + ", 배열길이 : "+tempArr.length);
		//문자열과  char배열의 + 연산을 실행하면 char 배열의 String 자동변환하지 않음.
		
		//[C@7a79be86 에서 7a79be86 는 해시코드값이고 해시 함수를 통해서 만들어지는 4바이트 값
